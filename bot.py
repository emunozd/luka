"""
bot.py — Bot de Telegram para LUKA
====================================
...
# ─────────────────────────────────────────────────────────────────────────────
# Comando desconocido y texto libre
# ─────────────────────────────────────────────────────────────────────────────
...
async def handle_texto_libre(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(texto) >= 50 and token:
        await update.message.reply_text("⏳ Analizando el texto como factura...")
        try:
            preview = _post("/facturas/texto/preview", {"texto": texto}, token=token)
            context.user_data[KEY_PREVIEW] = preview
            msg = _formatear_preview_texto(preview)
            keyboard = InlineKeyboardMarkup([[
                InlineKeyboardButton("✅ Guardar", callback_data="confirmar_texto"),
                InlineKeyboardButton("❌ Cancelar", callback_data="cancelar_preview"),
            ]])
            await update.message.reply_text(msg, parse_mode="Markdown", reply_markup=keyboard)
            return
        except Exception as e:
            logger.error("Error en handle_texto_libre: %s", e)
            await update.message.reply_text("❌ No se pudo procesar el texto.")
            return
